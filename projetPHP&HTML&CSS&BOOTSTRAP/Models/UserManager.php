...
class UserManager {
    ...
    public function checkConnection(User $new_user)
    {
        ...
        foreach($query as $user)
        {
            if (($new_user->login())==($user[1]) && password_verify($new_user->pwd(), $user[2]))
            {
                ...
                $checkConnect = True;
            } else if (($new_user->email())==($user[3]) && password_verify($new_user->pwd(),($user[2]))) 
            {
                $new_user->setID($user[0]);
                $new_user->setLogin($user[1]);
                $new_user->setPwd($user[2]);
    ...
    {
    $q = $this->_db->prepare('INSERT INTO user(login, pwd, email) VALUES(:login, :pwd, :email)');
    
    $q->bindValue(':login', $new_user->login());
    $q->bindValue(':pwd', $new_user->pwd());
    $q->bindValue(':email', $new_user->email());
